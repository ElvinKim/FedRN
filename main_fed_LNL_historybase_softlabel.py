...
from models.Update import LocalUpdateHS
from models.Nets import MLP, CNNMnist, CNNCifar, MobileNetCifar
from models.Fed import FedAvg
from models.test import test_img, test_img_HS

from utils.cifar import CIFAR10, CIFAR100, CIFAR10Basic, CIFAR10Softlabel
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        trans_cifar10_val = transforms.Compose([transforms.ToTensor(),
                                                transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                     std=[0.229, 0.224, 0.225])])
        dataset_train = CIFAR10Softlabel(root='./data/cifar',
                                        download=True,  
                                        train=True, 
                                        transform=trans_cifar10_train, 
                                         args=args)
        
        dataset_test = CIFAR10Softlabel(root='./data/cifar',
                                    download=True,  
                                    train=False, 
    ...
    # Set soft label
    for idx in range(len(dataset_train.train_data)):
        dataset_train.soft_labels[idx][dataset_train.train_labels[idx]] = 1.
    ...
    # warmup
    args.warmup = int(args.epochs * 0.2)
    ...
    if args.iid:
        result_f = 'HB_H_{}_{}_{}_C[{}]_BS[{}]_LE[{}]_IID[{}]_LR[{}]_MMT[{}]_NT[{}]_NGN[{}]_GNR[{}]_PT[{}]'.format(args.dataset, 
                                                                                                     args.model, 
                                                                                                     args.epochs, 
                                                                                                     args.frac, 
                                                                                                     args.local_bs, 
        ...
        result_f = 'HB_H_{}_{}_{}_C[{}]_BS[{}]_LE[{}]_IID[{}]_LR[{}]_MMT[{}]_NT[{}]_NGN[{}]_GNR[{}]'.format(args.dataset, 
                                                                                                     args.model, 
                                                                                                     args.epochs, 
                                                                                                     args.frac, 
                                                                                                     args.local_bs, 
    ...
    user_select_cnt_dict = {}
    
    for i in range(args.num_users):
        user_select_cnt_dict[i] = 0
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            local = LocalUpdateHS(args=args, dataset=dataset_train, idxs=dict_users[idx])
            
            if iter <= args.warmup:
                w, loss, dataset_train = local.warmup(net=copy.deepcopy(net_glob).to(args.device), s_cnt=user_select_cnt_dict[idx])
            else:
                w, loss, dataset_train = local.train(net=copy.deepcopy(net_glob).to(args.device), s_cnt=user_select_cnt_dict[idx])
            
            user_select_cnt_dict[idx] += 1
        ...
        # print result
        net_glob.eval()
        acc_train, loss_train = test_img_HS(net_glob, dataset_train, args)
        ...
        print("train acc: {}, train loss: {} \n test acc: {}, test loss: {}".format(acc_train.item(),
                                                                                    ...
                                                                                    acc_test.item(), 
                                                                                    loss_test))
        wr.writerow([iter + 1, acc_train, loss_train, acc_test.item(), loss_test])
    f.close()
