...
if __name__ == '__main__':
    ...
    for idx in range(args.num_users):
        w1_local_lst.append(copy.deepcopy(w_glob1))
        w2_local_lst.append(copy.deepcopy(w_glob2))
    ...
    result_f = 'fed_swap_coteaching_{}_{}_{}_C[{}]_IID[{}]_LR[{}]_MMT[{}]_NT[{}]_NR[{}]_Tk[{}]_RePr[{}].csv'.format(args.dataset, 
                                                                                       args.model, 
                                                                                       args.epochs, 
                                                                                       args.frac, 
                                                                                       args.iid,
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            if iter % 2 == 0:
                net_local.load_state_dict(w2_local_lst[idx])
                w_g, loss1, w_l, loss2 = local.train_coteaching(net1=copy.deepcopy(net_glob1).to(args.device), net2=copy.deepcopy(net_local).to(args.device), rate_schedule=rate_schedule[iter])
            else:
                net_local.load_state_dict(w1_local_lst[idx])
                w_g, loss1, w_l, loss2 = local.train_coteaching(net1=copy.deepcopy(net_glob2).to(args.device), net2=copy.deepcopy(net_local).to(args.device), rate_schedule=rate_schedule[iter])
            
            if args.all_clients:
                w_locals[idx] = copy.deepcopy(w_l)
            else:
        ...
        if iter % 2 == 0:
            w_glob2 = FedAvg(w_locals)
            net_glob2.load_state_dict(w_glob2)
        else:
            w_glob1 = FedAvg(w_locals)
