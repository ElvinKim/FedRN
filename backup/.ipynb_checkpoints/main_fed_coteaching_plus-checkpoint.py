...
from models.Update import LocalUpdate, LocalUpdateCoteachingPlus
...
if __name__ == '__main__':
    ...
    result_f = 'fedCoteaching_plus_{}_{}_{}_C[{}]_IID[{}]_LR[{}]_MMT[{}]_NT[{}]_NR[{}]_Tk[{}]_RePr[{}].csv'.format(args.dataset, 
                                                                                       args.model, 
                                                                                       args.epochs, 
                                                                                       args.frac, 
                                                                                       args.iid,
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            local = LocalUpdateCoteachingPlus(args=args, dataset=dataset_train, idxs=dict_users[idx])
            w1, loss1, w2, loss2 = local.train(net1=copy.deepcopy(net_glob1).to(args.device), 
                                                net2=copy.deepcopy(net_glob2).to(args.device), 
                                                rate_schedule=rate_schedule[iter], epoch=iter)
            if args.all_clients:
