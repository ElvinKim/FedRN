...
def get_local_update_objects(args, dataset_train, dict_users=None, noise_rates=None, net_glob=None, net_local_lst=None,
                             noise_logger=None, tmp_true_labels=None, gaussian_noise=None, user_noisy_data=None):
    ...
    for idx, noise_rate in zip(range(args.num_users), noise_rates):
        ...
        if args.method == 'default':
            ...
            local_update_object = LocalUpdateOurs(**local_update_args, gaussian_noise=gaussian_noise, user_noisy_data=user_noisy_data)
...
class LocalUpdateOurs(BaseLocalUpdate):
    def __init__(self, args, dataset=None, user_idx=None, idxs=None, noise_logger=None, tmp_true_labels=None,
        ...
        self.user_noisy_data = user_noisy_data[user_idx]
        self.user_clean_data = list(set(idxs)-set(user_noisy_data[user_idx]))
    ...
    def loss_dist(self, client_num, prev, neighbor_lst, neighbor_score_lst):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(DataLoader(dataset, batch_size=1, shuffle=False)):
                ...
                prev_loss = ce(prev_logits, labels)
                net = 'prev'
                ...
                for i, (n_net, score) in enumerate(zip(neighbor_lst, neighbor_score_lst)):
                    ...
                    n_loss = self.CE(n_outputs, labels) 
                    wr.writerow([self.args.g_epoch, net, self.user_idx, real_idx.item(), is_noise, n_loss.item()])
             
        f.close()
    
    def split_data_indices(self, prev, prev_score, neighbor_lst, neighbor_score_lst):
        ...
        idx_lst = []
        losses = [[] for i in range(1 + len(neighbor_lst))]
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets, items, idxs) in enumerate(self.ldr_eval):
                ...
                # Prev loss calculation
                outputs = prev(inputs)
                loss = self.CE(outputs, targets)
                losses_lst[0].append(loss)
        ...
        losses[0] = (losses[0] - losses[0].min()) / (losses[0].max() - losses[0].min())
        input_loss = losses[0].reshape(-1, 1)
        ...
        prev_precision, prev_recall = self.filtering_check(prob, indices)
        print('Client     - Precision: {:.5f}, Recall: {:.5f}'.format(prev_precision, prev_recall))
        threshold = 0.5
        pred = (prob > threshold)
        ...
        prob = [p * score_list[0] for p in prob]
        final_prob = np.add(final_prob, prob)
        
        nei_idx_lst = []
        # Neighbor fine-tuning with expected-to-be-clean data by prev_model
        neigbor_lst = self.finetune(neighbor_lst, clean_idx)
        
        if self.args.g_epoch in self.args.loss_dist_epoch:
            self.loss_dist(self.user_idx, self.net1, neighbor_lst, neighbor_score_lst)
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets, items, idxs) in enumerate(self.ldr_eval):
                inputs, targets = inputs.to(self.args.device), targets.to(self.args.device)
                for k, (n_net, score) in enumerate(zip(neighbor_lst, neighbor_score_lst)):
                    n_outputs = n_net(inputs)
                    loss = self.CE(n_outputs, targets)
                    losses_lst[k+1].append(loss)

        # Fit neighbor GMMs
        for i in range(len(losses_lst)-1):
            losses[i+1] = torch.cat(losses_lst[i+1]).cpu().numpy()
            losses[i+1] = (losses[i+1] - losses[i+1].min()) / (losses[i+1].max() - losses[i+1].min())
            input_loss = losses[i+1].reshape(-1, 1)
            gmm = GaussianMixture(n_components=2, max_iter=100, tol=1e-2, reg_covar=5e-4)
            gmm.fit(input_loss)
            prob = gmm.predict_proba(input_loss)
            prob = prob[:, gmm.means_.argmin()]
            precision, recall = self.filtering_check(prob, indices)
            net_name = 'Neighbor ' + str(i+1)
            print(net_name + ' - Precision: {:.5f}, Recall: {:.5f}'.format(precision, recall))
            # Apply score for weighted average
            prob = [p * score_list[i+1] for p in prob]
            final_prob = np.add(final_prob, prob)
       
        precision, recall = self.filtering_check(final_prob, indices)
        print('<Final>    - Precision: {:.5f}, Recall: {:.5f} (Precision {:+.5f} & Recall {:+.5f})'.format(precision, recall, precision-prev_precision, recall-prev_recall))
    ...
    def filtering_check(self, prob, indices):
    ...
    def train_phase2(self, client_num, net, prev_score, neighbor_lst, neighbor_score_lst):
        ...
        clean_idx, noisy_idx = self.split_data_indices(self.net1, prev_score, neighbor_lst, neighbor_score_lst)
