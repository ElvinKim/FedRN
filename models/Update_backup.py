...
class LocalUpdateLGFineTuningBackup(object):
    def __init__(self, args, dataset=None, idxs=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
    ...
    def train(self, g_net, l_net, forget_rate):
        ...
        # fine-tuning
        for _ in range(self.args.ft_local_ep):
            for batch_idx, (images, labels, indexes) in enumerate(self.ldr_train):
                images, labels = images.to(self.args.device), labels.to(self.args.device)
                l_net.zero_grad()
        ...
        iter_num = int(self.args.epochs / self.args.local_bs)
...
class LocalUpdateLGFineTuningBackup2(object):
    def __init__(self, args, dataset=None, idxs=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
    ...
    def train(self, g_net, l_net, forget_rate):
        ...
        update_keys = [k for k in g_net.state_dict().keys() if ('linear' not in k and 'layers.12' not in k)]
        g_update_w = {k: g_w[k] for k in update_keys}
        
        
        # receive body parts of global
        l_net.load_state_dict(g_update_w, strict=False)
        ...
        iter_num = int(len(self.idxs) / self.args.local_bs)
...
class LocalUpdateLGFineTuning(object):
    def __init__(self, args, dataset=None, idxs=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.loss_func_none = nn.CrossEntropyLoss(reduction='none')
        self.selected_clients = []
        self.dataset = dataset
        self.ldr_train = DataLoader(DatasetSplit(dataset, idxs), batch_size=self.args.local_bs, shuffle=True)
        self.idxs = idxs
    ...
    def train(self, g_net, l_net, forget_rate):
        ...
        update_keys = [k for k in g_net.state_dict().keys() if ('linear' not in k)]
        g_update_w = {k: g_w[k] for k in update_keys}
        ...
        for local_ep in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                images, labels = images.to(self.args.device), labels.to(self.args.device)
                g_net.zero_grad()
                g_log_probs = g_net(images)
                l_log_probs = l_net(images)
                loss_g, loss_l = loss_coteaching(g_log_probs, l_log_probs, labels, forget_rate, self.loss_func_none)
                loss_g.backward()
                g_optimizer.step()
...
#                 unlabeled_idxs.append(labeled_idxs.pop(-1))
        ...
        for ep in range(self.args.local_ep):
            ...
            for batch_idx in range(self.num_iter):
                ...
                Lx, Lu, lamb = criterion(logits_x, mixed_target[:batch_size*2], logits_u, mixed_target[batch_size*2:], g_epoch+batch_idx/self.num_iter, self.args.warm_up)                
                # regularization
...
class LocalUpdateDivideMix(object):
    ...
    def train(self, g_epoch, warmup, net1, net2, all_loss):
        ...
        prob1, all_loss[0], prob_dict1, label_idx1, unlabel_idx1 = self.eval_train(net1, all_loss[0], p_threshold=self.args.p_threshold)
        prob2, all_loss[1], prob_dict2, label_idx2, unlabel_idx2 = self.eval_train(net2, all_loss[1], p_threshold=self.args.p_threshold)    
    ...
    def divide_mix(self, epoch, warm_up, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader):
        ...
        for batch_idx in range(num_iter):
            ...
            Lx, Lu, lamb = criterion(self.args, logits_x, mixed_target[:batch_size*2], logits_u, mixed_target[batch_size*2:], epoch+batch_idx/num_iter, warm_up)
    ...
    def warmup(self, net, s_cnt):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels, soft_labels, predictions, indexes) in enumerate(ldr_train):
                images, labels, soft_labels = images.to(self.args.device), labels.to(self.args.device), soft_labels.to(self.args.device)
                net.zero_grad()
                log_probs = net(images)
...
class LocalUpdateRFL(object):
    def __init__(self, args, dataset=None, idxs=None, glob_centroid=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
        self.ldr_train = DataLoader(DatasetSplit(dataset, idxs), batch_size=self.args.local_bs, shuffle=True)
        self.glob_centroid = glob_centroid
    ...
    def train(self, net, mu=0.01):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                #for fedprox
                fed_prox_reg = 0.0
                for l_param, g_param in zip(net.parameters(), glob_net.parameters()):
                    fed_prox_reg += (mu/2  * torch.norm((l_param - g_param))**2)
