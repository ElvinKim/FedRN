...
from models.Update import LocalUpdate, LocalUpdateJO
from models.Nets import MLP, CNNMnist, CNNCifar, MobileNetCifar
from models.Fed import FedAvg
from models.test import test_img, test_img_JO

from utils.cifar import CIFAR10, CIFAR100, CIFAR10JointOpt
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        dataset_train = CIFAR10JointOpt(root='./data/cifar',
                                    download=True,  
                                    train=True, 
        ...
        clean_dataset_train = CIFAR10JointOpt(root='./data/cifar',
                                    download=True,  
                                    train=True, 
                                    transform=trans_cifar10_train,
                                    noise_type="clean"
    ...
    result_f = 'fedLNL_JointOpt_{}_{}_{}_C[{}]_IID[{}]_LR[{}]_MMT[{}]_NT[{}]_NR[{}]_BEGIN[{}]_ALPHA[{}]_BETA[{}].csv'.format(args.dataset, 
                                                                                                                    args.model, 
                                                                                                                    args.epochs, 
                                                                                                                    args.frac, 
                                                                                                                    args.iid,
    ...
    results = np.zeros((len(dataset_train), 10), dtype=np.float32) 
    ...
    for iter in range(args.epochs):
        ...
        idx = idxs_users[0]
        tmp_l = list(dict_users[idx])
        tmp_l.sort()
        i = tmp_l[0]
        
        print(dataset_train[i][2])
        
        for idx in idxs_users:
            local = LocalUpdateJO(args=args, dataset=dataset_train, idxs=dict_users[idx], results=results)
            w, loss, results, dataset_train = local.train(net=copy.deepcopy(net_glob).to(args.device), g_epoch=iter)
            if args.all_clients:
        ...
        # print result
        net_glob.eval()
        acc_train, loss_train = test_img_JO(net_glob, dataset_train, args)
